...
std::vector<std::vector<int>> generate_negative_samples(const std::vector<std::vector<int>>& edge_index, 
    ...
    for(int i = 0; i < pos_edge_index[0].size(); i++){
        ...
        // std::set_difference(nodeset.begin(), nodeset.end(), unavail_nodes.begin(), unavail_nodes.end(), std::inserter(avail_nodes, avail_nodes.begin()));
        // for(int j = 0; j < num_neg_samples; j++){
        //     int neg_node = *std::next(avail_nodes.begin(), j);
        //     if(j < num_neg_samples/2){
