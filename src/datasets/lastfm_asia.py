...
class LastFMAsia():
        def __init__(self, root, mask_type="replace", pretrain: set[PretrainType] = set(), split_type='random', splits=[0.6, 0.2, 0.2], khop_neighbors=[100, 100], ports=False, ego=False, channels=64):
            ...
            self.edges = LastFMAsiaEdges(os.path.join(root, 'lastfm_asia_edges.csv'), ports=ports)
            self.edges.materialize()
            self.edges.init_encoder(channels)
            logger.info(f'Edges created.')
            logger.info(f'Creating nodes...')
            self.nodes = LastFMAsiaNodes(os.path.join(root, 'lastfm_asia_nodes.csv'), mask_type=mask_type, pretrain=pretrain, split_type=split_type, splits=splits, khop_neighbors=khop_neighbors, ego=ego)
            self.nodes.materialize()
            self.nodes.init_encoder(channels)
            logger.info(f'Nodes created.')
            self.num_classes = self.nodes.n_classes
...
class LastFMAsiaEdges(torch_frame.data.Dataset):
        def __init__(self, root, khop_neighbors=[100,100], ports=False):
            ...
            start = time.time()
            col_to_stype = create_graph(self, col_to_stype, "node_1", "node_2")
...
class LastFMAsiaNodes(torch_frame.data.Dataset):
        def __init__(self, root, mask_type="replace", pretrain: set[PretrainType] = set(), split_type='random', splits=[0.6, 0.2, 0.2], khop_neighbors=[100, 100], ego=False):
            ...
            self.num_columns = [col for col in self.df.columns if col != 'target' and col != 'id']
            self.cat_columns = []
            ...
            if PretrainType.MASK in pretrain or PretrainType.LINK_PRED in pretrain:
                ...
                self.df['target'] = self.df.apply(lambda row: [row['target'], row['id']], axis=1)
