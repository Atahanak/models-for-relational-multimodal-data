...
class OgbnArxiv():
    def __init__(self, root, mask_type="replace", pretrain: set[PretrainType] = set(), split_type='random', splits=[0.6, 0.2, 0.2], khop_neighbors=[100, 100], ports=False, ego=False, channels=64):
        ...
        self.edges = OgbnArxivEdges(os.path.join(root, 'edges.csv'), ports=ports)
        self.edges.materialize()
        self.edges.init_encoder(channels)
        logger.info(f'Edges created.')
        logger.info(f'Creating nodes...')
        self.nodes = OgbnArxivNodes(os.path.join(root, 'nodes.csv'), mask_type=mask_type, pretrain=pretrain, split_type=split_type, splits=splits, khop_neighbors=khop_neighbors, ego=ego)
...
class OgbnArxivEdges(torch_frame.data.Dataset):
    def __init__(self, root, khop_neighbors=[100,100], ports=False):
        ...
        start = time.time()
        col_to_stype = create_graph(self, col_to_stype, "src", "dst")
        ...
        if self.ports:
            ...
            start = time.time()
            add_ports(self)
            col_to_stype['in_port'] = stype.relation
            col_to_stype['out_port'] = stype.relation
            logger.info(f'Ports added in {time.time()-start:.2f} seconds.')
        else:
            self.df['edge_attr'] = 1
            col_to_stype['edge_attr'] = stype.relation
...
class OgbnArxivNodes(torch_frame.data.Dataset):
    def __init__(self, root, mask_type="replace", pretrain: set[PretrainType] = set(), split_type='temporal', splits=[0.6, 0.2, 0.2], khop_neighbors=[100, 100], ego=False):
        ...
        for col in self.df.columns:
            if col != 'index' and col != 'label' and col != 'id':
                col_to_stype[col] = torch_frame.numerical

        self.num_columns = [col for col in self.df.columns if col != 'index' and col != 'label' and col != 'id']
        self.cat_columns = []
        ...
        if PretrainType.MASK in pretrain or PretrainType.LINK_PRED in pretrain:
            ...
            self.df['target'] = self.df.apply(lambda row: [row['label'], row['id']], axis=1)
            self.target_col = 'target'
            col_to_stype['target'] = stype.relation
        if self.ego:
            self.df['EgoID'] = 0
