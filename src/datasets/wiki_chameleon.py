...
class WikiChameleon():
        def __init__(self, root, mask_type="replace", pretrain: set[PretrainType] = set(), split_type='random', splits=[0.6, 0.2, 0.2], khop_neighbors=[100, 100], ports=False, ego=False, channels=64):
            ...
            self.edges = WikiChameleonEdges(os.path.join(root, 'musae_chameleon_edges.csv'), ports=ports)
            self.edges.materialize()
            self.edges.init_encoder(channels)
            logger.info(f'Edges created.')
            logger.info(f'Creating nodes...')
            self.nodes = WikiChameleonNodes(os.path.join(root, 'musae_chameleon_nodes.csv'), mask_type=mask_type, pretrain=pretrain, split_type=split_type, splits=splits, ego=ego)
...
class WikiChameleonEdges(torch_frame.data.Dataset):
...
class WikiChameleonNodes(torch.utils.data.Dataset):
        ...
        def split(self):
            ...
            return WikiChameleonNodes(df=self.df[self.df['split'] == 0]), WikiChameleonNodes(df=self.df[self.df['split'] == 1]), WikiChameleonNodes(df=self.df[self.df['split'] == 2])
