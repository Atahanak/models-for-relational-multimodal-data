...
run = wandb.init(
    ...
    project=f"rel-mm",
    name="model=fttransformer,dataset=IBM-AML_Hi_Sm,objective=MCM-bert,loss=weighted_loss",
    config=args,
    group="bert_mask",
)
...
dataset = IBMTransactionsAML(root=root, mask_type="bert", mask=mask, pretrain=pretrain, split_type='temporal', splits=data_split)
ic(dataset)
dataset.materialize()
