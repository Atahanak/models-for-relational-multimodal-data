...
logging.info(sweep_config)
sweep_id = wandb.sweep(sweep_config, entity='cse3000', project='rel-mm-supervised')
...
def run():
    ...
    with wandb.init(
        ...
        project="rel-mm-supervised",
        entity="cse3000",
        config=config
    ):

        wconfig = wandb.config
        config["lr"] = wconfig.lr
        config["dropout"] = wconfig.dropout
        config["n_gnn_layers"] = wconfig.n_gnn_layers
        config["n_hidden"] = wconfig.n_hidden
...
wandb.agent(sweep_id, run, count=54)
wandb.finish()
