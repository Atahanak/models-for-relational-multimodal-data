...
run = wandb.init(
    ...
    project=f"rel-mm",
    group="MCM-replace+MV",
    name=f"model=fttransformer,dataset=IBM-AML_Hi_Sm,objective=MCM-replace+MV,channels={channels},weight_decay={weight_decay}",
    config=args
)
...
dataset = IBMTransactionsAML(root='/scratch/imcauliffe/AML_dataset_preprocessed/HI-Small_Trans-c.csv', pretrain=pretrain, mask_type="replace")
...
def train(epoc: int) -> float:
    ...
    with tqdm(train_loader, desc=f'Epoch {epoc}') as t:
        for tf in t:
            ...
            t_c += loss_c[1]
            t_n += loss_n[1]
            t.set_postfix(loss=f'{loss_accum / total_count:.4f}', mcm_loss=mcm_loss, mv_loss=mv_loss, loss_c=f'{loss_c_accum / t_c:.4f}',
            ...
            del mv_out
            del tf
            wandb.log({"train_loss": loss_accum / total_count, "train_loss_c": loss_c_accum / t_c,
                       "train_loss_n": loss_n_accum / t_n, "train_mcm_loss": mcm_loss, "train_mv_loss": mv_loss})
    ...
    with tqdm(loader, desc=f'Evaluating') as t:
        for tf in t:
            ...
            for i, ans in enumerate(tf.y):
                ...
                if ans[1] > (num_numerical - 1):
                    accum_acc += (cat_out[int(ans[1]) - num_numerical][i].argmax() == int(ans[0]))
                else:
                    accum_l2 += torch.square(ans[0] - num_out[i][int(ans[1])])  # rmse
            # MV accuracy
            mv_accuracy = ssmetric.mv_accuracy(mv_out, tf.y)
            ...
            t.set_postfix(accuracy=f'{accum_acc / t_c:.4f}', mv_accuracy=f'{mv_accuracy:.4f}', rmse=f'{torch.sqrt(accum_l2 / t_n):.4f}',
                          mcm_loss=f'{(loss_c_accum / t_c) + (loss_n_accum / t_n):.4f}', mv_loss=mv_loss, loss_c=f'{loss_c_accum / t_c:.4f}',
                          loss_n=f'{loss_n_accum / t_n:.4f}')
        wandb.log({f"{dataset_name}_accuracy": accum_acc / t_c, f"{dataset_name}_rmse": torch.sqrt(accum_l2 / t_n),
                   f"{dataset_name}_loss": ((loss_c_accum / t_c) * (num_categorical / num_columns)) + (
                               (loss_n_accum / t_n) * (num_numerical / num_columns)),
                   f"{dataset_name}_loss_c": loss_c_accum / t_c, f"{dataset_name}_loss_n": loss_n_accum / t_n,
                   f"{dataset_name}_mv_accuracy": mv_accuracy, f"{dataset_name}_mv_loss": mv_loss})
        del tf
        del num_out
        del cat_out
        del mv_out
