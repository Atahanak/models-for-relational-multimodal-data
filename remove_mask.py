...
run = wandb.init(
    ...
    project=f"rel-mm",
    name="model=fttransformer,dataset=IBM-AML_Hi_Sm,objective=MCM-remove,loss=weighted_loss",
    config=args,
    group="remove_mask",
)
...
dataset = IBMTransactionsAML(root=root, mask_type="remove", mask=mask, pretrain=pretrain, split_type='temporal', splits=data_split)
ic(dataset)
dataset.materialize()
