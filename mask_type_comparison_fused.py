...
mask_types = ["remove", "replace", "bert"]
for mask_type in mask_types:
    ...
    run = wandb.init(
        ...
        project=f"rel-mm",
        name=f"model=FTTransformerGINeFusedEdgeUpdates,dataset=IBM-AML_Hi_Sm,objective=lp+mask({mask_type}),khop_neighs=[100,100],channels={channels},epoch=3,nointnode",
        #name=f"debug-fused",
        config=args
    )
