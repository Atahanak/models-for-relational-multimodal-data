...
# %%
def run_mask_strategy(strategy="replace"):
    seed = 42
    batch_size = 1024
    channels = 32
    num_layers = 4
    ...
    run = wandb.init(
        ...
        project=f"rel-mm",
        name=f"model=fttransformer,dataset=IBM-AML_Hi_Sm,objective=MCM-{strategy},loss=weighted_loss",
        config=args,
        group=f"{strategy}_mask",
    )
    ...
    dataset = IBMTransactionsAML(root=root, mask_type=strategy, pretrain=pretrain, split_type='temporal',
                                 splits=data_split)
    ic(dataset)
    dataset.materialize()
        ...
        with tqdm(loader, desc=f'Evaluating') as t:
            for tf in t:
                ...
                for i, ans in enumerate(tf.y):
                    ...
                    # pred --> feature_type_num X type_num X batch_size
                    if ans[1] > (num_numerical - 1):
                        accum_acc += (pred[1][int(ans[1]) - num_numerical][i].argmax() == int(ans[0])).item()
                    else:
                ...
                t.set_postfix(accuracy=f'{accum_acc / t_c:.4f}', rmse=f'{torch.sqrt(accum_l2 / t_n):.4f}',
                              loss=f'{(loss_c_accum / t_c) + (loss_n_accum / t_n):.4f}', loss_c=f'{loss_c_accum / t_c:.4f}',
                              loss_n=f'{loss_n_accum / t_n:.4f}')
            ...
            rmse = torch.sqrt(accum_l2 / t_n).item()
            del accum_l2
            del accum_acc
