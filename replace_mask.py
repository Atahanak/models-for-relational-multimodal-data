...
run = wandb.init(
    ...
    project=f"rel-mm",
    name="model=fttransformer,dataset=IBM-AML_Hi_Sm,objective=MCM-replace,loss=weighted_loss",
    config=args,
    group="replace_mask",
)
...
try:
    mask = np.load("/scratch/imcauliffe/masked_columns")
except:
    N = pd.read_csv(root).shape[0]
    maskable_columns = ['Amount Received', 'Receiving Currency', 'Amount Paid', 'Payment Currency', 'Payment Format']
    mask = np.random.choice(maskable_columns, size=N, replace=True)
    np.save("/scratch/imcauliffe/masked_columns", mask)

dataset = IBMTransactionsAML(root=root, mask_type="replace", mask=mask, pretrain=pretrain, split_type='temporal', splits=data_split)
ic(dataset)
dataset.materialize()
